...
5. **IDs auto-gerados pelo Firestore**, exceto quando precisar de slug/code (cupom, produtor slug).
...
```
users ────┐
          ├─ owns ──> produtores ──┬─ contains ──> products
          │                          ├─ contains ──> categories
          │                          └─ has ─────── stats/{date}
          │
...
interface User {
  ...
  // Específico por role
  produtorIds?: string[]     // produtor: hortas que gerencia
...
#### Subcollections de `produtores`

##### `produtores/{produtorId}/categories/{categoryId}`
...
##### `produtores/{produtorId}/products/{productId}`
...
##### `produtores/{produtorId}/stats/{yyyy-MM-dd}`
...
interface Order {
  ...
  pricing: {
    ...
    produtorNet: number      // o que o produtor recebe
  }
  ...
  cancellation?: {
    reason: string
    cancelledBy: 'customer' | 'produtor' | 'admin' | 'system'
...
type OrderStatus =
  ...
  | 'confirmed'                // pagamento OK, esperando produtor aceitar
...
interface StatusChange {
  status: OrderStatus
  at: Timestamp
  by: 'system' | 'customer' | 'produtor' | 'admin'
  byUid?: string
  reason?: string
}
```
...
interface AuditLog {
  ...
  adminUid: string
  adminEmail: string
  action: string               // 'produtor.approve', 'order.cancel', ...
  targetType: string
  targetId: string
...
orders:
  ...
  - (produtorId ASC, status ASC, createdAt DESC) # pedidos do produtor
...
Para "produtores próximos", usar uma das duas estratégias:
...
Adicionar campo `geohash` em `produtores` e usar a lib `geofire-common` para encontrar candidatos. Não é perfeito mas escala muito melhor.
...
- **`orders` carrega snapshot do cliente, produtor e endereço** no momento do pedido. Se produtor mudar nome depois, pedidos antigos mantêm o nome original.
- **`orders.items` tem snapshot do produto** (nome, foto, preço). Se produto for excluído, pedido continua válido.
- **`produtores.stats.rating`** é desnormalizado — atualizado via Cloud Function quando avaliação é criada.
- **`coupons.usedCount`** é incrementado atomicamente via `FieldValue.increment(1)`.

## Padrões de exclusão

- **Produtores:** soft delete (`deletedAt`). Histórico de pedidos continua referenciando.
