...
export default function NotificationBell() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      console.error('notifications error:', err.code)
    })
  ...
  return (
    <div className="relative" ref={dropRef}>
      ...
      <button
        ...
        className="relative rounded-xl p-2 text-neutral-400 transition-colors hover:bg-neutral-100 hover:text-neutral-700"
      >
            ...
            {notifs.length === 0 ? (
              ...
              notifs.map((n) => (
                <button
                  ...
                  className={[
                    'flex w-full gap-3 px-4 py-3 text-left text-sm transition-colors hover:bg-neutral-50',
                    !n.read ? 'bg-blue-50/50' : '',
                  ].join(' ')}
                >
                  <span className="mt-0.5 shrink-0 text-base">{STATUS_ICON[n.status] ?? '🔔'}</span>
                  <p className={['flex-1 min-w-0 leading-snug text-neutral-700', !n.read ? 'font-medium' : ''].join(' ')}>
                    {n.message}
                  </p>
