...
export default function HortaCatalog({
  ...
  const [activeTab, setActiveTab] = useState<string>(initialProdutores[0]?.id ?? '')
  ...
  function handleAddItem(product: SerializableProduct) {
    ...
    const result = addItem(
      product as Product,
      { id: activeProdutor.id, name: activeProdutor.name },
      cartHorta,
    )
    if (result === 'conflict') {
  ...
  return (
    <>
      ...
      {/* Tabs de produtor — só exibe se houver mais de um */}
      {produtores.length > 1 && (
